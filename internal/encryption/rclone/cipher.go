...
import (
	...
	"github.com/WhispersOfJ/bearmount/internal/encryption/rclone/pkcs7"
	"github.com/WhispersOfJ/bearmount/internal/encryption/rclone/version"
...
func NewCipher(mode NameEncryptionMode, password, salt string, dirNameEncrypt bool, enc fileNameEncoding) (*Cipher, error) {
	...
	if password != "" {
		k, err := GenerateKey(password, salt)
		if err != nil {
			return nil, err
		}
...
func (c *Cipher) encryptSegment(plaintext string, k *key) string {
	if plaintext == "" {
		return ""
	}
	paddedPlaintext := pkcs7.Pad(nameCipherBlockSize, []byte(plaintext))
	ciphertext := eme.Transform(k.block, k.nameTweak[:], paddedPlaintext, eme.DirectionEncrypt)
	return c.fileNameEnc.EncodeToString(ciphertext)
}

// decryptSegment decrypts a path segment
func (c *Cipher) decryptSegment(ciphertext string, k *key) (string, error) {
	if ciphertext == "" {
		return "", nil
	}
	...
	paddedPlaintext := eme.Transform(k.block, k.nameTweak[:], rawCiphertext, eme.DirectionDecrypt)
...
func (c *Cipher) obfuscateSegment(plaintext string, k *key) string {
	if plaintext == "" {
		return ""
	}
...
func (c *Cipher) deobfuscateSegment(ciphertext string, k *key) (string, error) {
	if ciphertext == "" {
		return "", nil
	}
...
func (c *Cipher) encryptFileName(in string, k *key) string {
	...
	for i := range segments {
		...
		if c.mode == NameEncryptionStandard {
			segments[i] = c.encryptSegment(segments[i], k)
		} else {
			segments[i] = c.obfuscateSegment(segments[i], k)
		}
...
func (c *Cipher) EncryptFileName(in string, k *key) string {
	...
	return c.encryptFileName(in, ku)
}
...
func (c *Cipher) decryptFileName(in string, k *key) (string, error) {
	...
	for i := range segments {
		...
		if c.mode == NameEncryptionStandard {
			segments[i], err = c.decryptSegment(segments[i], k)
		} else {
			segments[i], err = c.deobfuscateSegment(segments[i], k)
		}
...
func (c *Cipher) DecryptFileName(in string, k *key) (string, error) {
	...
	return c.decryptFileName(in, ku)
}
...
func (c *Cipher) newEncrypter(in io.Reader, nonce *nonce, k *key) (*encrypter, error) {
	fh := &encrypter{
		in:      in,
		c:       c,
		buf:     c.getBlock(),
...
func (fh *encrypter) Read(p []byte) (n int, err error) {
	...
	if fh.bufIndex >= fh.bufSize {
		...
		secretbox.Seal((*fh.buf)[:0], readBuf[:n], fh.nonce.pointer(), &fh.k.dataKey)
		fh.bufIndex = 0
...
func (c *Cipher) encryptData(in io.Reader, k *key) (io.Reader, *encrypter, error) {
	out, err := c.newEncrypter(in, nil, k)
	if err != nil {
		return nil, nil, err
	}
	return out, out, nil // and wrap the accounting back on
}
...
func (c *Cipher) newDecrypter(rc io.ReadCloser, k *key) (*decrypter, error) {
	fh := &decrypter{
		rc:      rc,
		c:       c,
		buf:     c.getBlock(),
...
func (c *Cipher) newDecrypterSeek(ctx context.Context, open OpenRangeSeek, offset, limit int64, k *key) (fh *decrypter, err error) {
...
func (fh *decrypter) fillBuffer() (err error) {
	...
	_, ok := secretbox.Open((*fh.buf)[:0], (*readBuf)[:n], fh.nonce.pointer(), &fh.k.dataKey)
	if !ok {
		if err != nil && err != io.EOF {
			return err // return pending error as it is likely more accurate
		}
...
func (c *Cipher) DecryptDataSeek(ctx context.Context, open OpenRangeSeek, offset, limit int64, k *key) (ReadSeekCloser, error) {
	...
	out, err := c.newDecrypterSeek(ctx, open, offset, limit, ku)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func GenerateKey(password, salt string) (*key, error) {
	...
	const keySize = len(k.dataKey) + len(k.nameKey) + len(k.nameTweak)
	...
	copy(k.dataKey[:], key)
	copy(k.nameKey[:], key[len(k.dataKey):])
	copy(k.nameTweak[:], key[len(k.dataKey)+len(k.nameKey):])
	// Key the name cipher
	k.block, err = aes.NewCipher(k.nameKey[:])
	return k, err
}
